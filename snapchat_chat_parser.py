        ...
        if year_created < 2024:
            ...
            for chat_type in full_chat_history.keys():
                ...
                for chat in full_chat_history[chat_type]:
                    ...
                    if chat_type.startswith("Received"):
                        ...
                        raise ValueError(f"Unknown value for chat type: '{chat_type}'")
                ...
                print(f"Finished analyzing {len(full_chat_history[chat_type])} {chat_type} messages")
            ...
            for username in full_chat_history.keys():
                ...
                if len(username) > 1:
                    ...
                    for chat in full_chat_history[username]:
                        ...
                        if chat["Media Type"] == "TEXT":
                            ...
                            if chat["IsSender"] == True:
                                ...
                                raise ValueError(f"Unknown value for `IsSender` key, expected only true or false, got: {chat['IsSender']}")
                    ...
                    print(f"Finished analyzing {len(parsed_chat_history[username])} messages from {username}")
...
if __name__ == "__main__":
    ...
    argparse_parser.add_argument("-i", "--input-chat-history-json", required=True, type=str, help="The path to the `chat_history.json` file")

    argparse_parser.add_argument("-y", "--year", required=True, type=int, help="The year the snapchat dump was taken. Needed to determine format.")

    argparse_parser.add_argument("-o", "--output-dir", required=True, type=str, help="The output directory to put the JSON files")
    ...
    if os.path.exists(chat_history_filepath):
        ...
        parsed_chat_history = parse_chats_from_file(chat_history_filepath, argparse_args.year)
        export_messages_to_files(parsed_chat_history, argparse_args.output_dir)

    else:
        raise FileNotFoundError(f"Error couldn't find chat history file: '{chat_history_filepath}'")
